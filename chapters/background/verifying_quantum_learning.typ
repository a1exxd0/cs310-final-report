...
@sec:fourier-analysis, the central question becomes: can a classical verifier, with access to only
classical random examples or statistical queries, reliably delegate an agnostic learning task to an
untrusted quantum prover?
...
candidate hypothesis is near-optimal requires knowledge of the best-in-class error, which is what
makes the learning problem hard.
